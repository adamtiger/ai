...
class JSONwriter:
    ...
    def __model_to_JSON(self, model):
        ...
        layer_input = {'layer':'Input2D', 'width':inp_sizes[1], 'height':inp_sizes[2], 'channel':inp_sizes[3]}
    ...
    def __get_layer(self, layer_descr):
        ...
        if 'Convolution2D' == name:
            k_h = layer_descr['config']['nb_row']
            k_w = layer_descr['config']['nb_col']
            k_num = layer_descr['config']['nb_filter']
            s_h = layer_descr['config']['subsample'][0]
            s_v = layer_descr['config']['subsample'][1]
            ...
            layers.append({'layer':'Dense2D', 'units':layer_descr['config']['output_dim']})
            if layer_descr['config']['bias']:
                layers.append({'layer':'Bias2D', 'units':layer_descr['config']['output_dim']})
            return layers
             
        elif 'MaxPooling2D' == name:
            k_h = layer_descr['config']['pool_size'][1]
            k_w = layer_descr['config']['pool_size'][0]
            s_h = layer_descr['config']['strides'][0]
            s_v = layer_descr['config']['strides'][1]
